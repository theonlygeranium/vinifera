...
- Add a default-disabled, main-only protected Gate 13 ShipCompliant sandbox
  acceptance controller and bind it to the exact canonical `staging` head,
  reviewed Supabase runtime hash, and hashed ShipCompliant
  values reported by the deployed Worker; brand-scope the cross-tenant fixture;
  require timeout-specific runtime evidence; and make completed-label recovery
  independent of current purchase credentials and origin configuration. Run
  the main-only controller through its dedicated main-branch environment,
  reject nonnumeric attempt counts and recovered rates, validate recovered
  evidence before analytics, and prove the reusable recovery fixture is
  unchanged. Propagate a structured timeout classification so DNS, TLS, and
  connection failures cannot satisfy the timeout scenario. Bind each label
  denial to its exact status and decision identity, derive analytics keys with
  the Worker's stable hash contract, accept exact pre-existing compliance and
  analytics timeout evidence on safe retries, and reject Supabase redirects before attaching Access
  headers. Snapshot the fingerprint fixture identity and address before its
  compliance request so interrupted runs can always restore the documented
  baseline; cleanup must affect one tenant-scoped shipment, explicitly clear
  all compliance state even when the address never changed, and leave its
  compliance pointer/status cleared. Revalidate the exact staging Worker
  revision and every reviewed Supabase/ShipCompliant runtime binding after
  every scenario and audit read before evidence can pass. The
  existing completed-label recovery
  RPC now accepts its intended `label_created` state, validates recovered
  provider evidence, and is callable only through an explicit tenant/brand-
  scoped overload used by both acquisition paths. Raise the verified Vitest
  floor to 591 tests.
  Re-fetch canonical `main` and `staging` immediately after acceptance and
  fail closed with non-passing evidence when either ref advanced; acquire the
  brand-scoped shipment row with an exclusive lock before delegating so
  concurrent label recovery cannot deadlock during a lock upgrade. Route the
  shared Phase 4 pgTAP callers through a schema-aware adapter so linked current
  staging exercises the brand-scoped overload while point-in-time QA remains
  valid against the historical six-argument schema. Keep only stable provider
  and environment target hashes in the checked-in Gate 13 policy and move the
  exact candidate-bound fixture-manifest hash to protected post-immutable
  per-run state, eliminating the candidate-SHA fixed point while retaining
  exact-byte authorization. Pass an independently selected expected brand
  through linked current-schema pgTAP, require an organization/brand/shipment
  match before the seven-argument RPC, and add wrong-brand negative coverage;
  the verified Phase 4 database total is now 160 assertions. Classify deadline
  aborts raised while streaming either OAuth or compliance response JSON as
  timeout evidence instead of swallowing them as malformed provider bodies.
  Permit the Gate 13 control suite to validate either the empty disabled policy
  or a complete reviewed enabled policy so activation does not require a test
  rewrite.
  The dedicated GitHub environment is created with its `main` branch policy,
  exact staging Worker origin, and one-shot toggle set to `false`; provider and
  acceptance secrets remain unprovisioned.
  **Deployment impact:** strengthens protected staging evidence and the existing
  recovery path; no provider account, credential, fixture, Worker, database,
  billing, DNS, mobile-store, production mutation, or gate-completion claim.
### Changed
- Add a default-disabled, main-only protected Gate 13 ShipCompliant sandbox
  acceptance controller and bind it to the exact canonical `staging` head,
  reviewed Supabase runtime hash, and hashed ShipCompliant
