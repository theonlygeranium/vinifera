...
- Add a default-disabled, protected Phase 4 hosted acceptance controller with
  independent one-shot switches for Gates 10, 11, and 12. It binds sanitized
  evidence to the exact staging revision, organization, brand, Worker,
  Supabase target, and protected post-immutable manifest hash; requires exact analytics/CSV
  reconciliation with hashed active-winery provenance and synthetic data
  exclusion, the full ML population/six-source/AUC/30-day superiority contract
  with the database qualification evidence hash recomputed from its exact
  run/dataset/status payload plus exact denominator, six-source counts, and
  reconciliation horizon, or Estate/Reserve cohort-wide owner opt-in and
  per-contributor entitlement/ten-contributor/privacy/quarterly delivery whose
  recomputed relationship hash binds organization, brand, cohort, period,
  report, and provider message
  contract respectively; binds the manifest and runtime-reported Supabase
  target to the exact candidate; rejects reversed source windows and future
  evidence or benchmark delivery; and retains a 90-day artifact without treating
  readiness as completion. **Deployment impact:** trusted main-only staging
  acceptance workflow and disabled target policy only; no hosted mutation,
  model promotion, benchmark publication, provider send, or production change.
  The controller script itself is authority-high-risk, and Gate 11 additionally
  requires consecutive per-day full scoring/assignment coverage plus a reviewed
  power-analysis threshold met by the observed outcomes and reconciliation
  through the completed outcome horizon. Gate 12 now requires both ends of its
  source window to fall inside the declared report quarter. All evidence
  instants must be timezone-qualified ISO/RFC3339 values, bounded basis-point
  proportions are capped at 10,000 without incorrectly capping ratios or
  per-member activity counts, and cancellations cannot exceed the eligible
  population.
  Canonical main/staging refs are force-refreshed before acceptance and again
  before artifact retention; drift or ref-refresh failure rewrites the report
  as failed, while rewrite failure removes it from the uploader path.
  Runtime health is re-probed after the configuration/database binding to
  reject evidence spliced across a concurrent deploy or rollback.
  Per-run manifest authorization moved from the checked-in candidate policy to
  the protected acceptance environment after the candidate is immutable,
  removing the candidate-SHA self-reference while retaining exact-byte and
  exact-revision checks. Gate 12 now requires one unique hashed owner opt-in
  audit and organization identity tied to the exact cohort for every
  contributor, including the selected winery, requires an Estate/Reserve tier
  and positive entitlement attestation for each contributor, and counts unique organizations
  rather than brands. Gate 11 now binds the qualified training run, selected
  model version, completed experiment, and promotion audit through explicit
  immutable IDs and matching relationship fields. The exact experiment record
  must contain at least 50 evaluated outcomes, matching the database promotion
  constraint; its real RPC-produced timestamps define inclusive UTC start and
  completion dates so neither partial boundary day can escape coverage. Its
  pre-promotion model state/AUC, completed same-experiment AUC and Brier
  superiority, and latest model-bound non-retraining drift report now mirror
  the remaining database promotion predicates, including the seven-day drift
  freshness window. The qualification reconciliation horizon is normalized to
  the database tool's ISO calendar-date granularity. Gate 12 now requires its
  recomputed delivery relationship to bind the persisted delivered email-log
  state and immutable confirmed provider-delivery event identity/status, not
  only a provider message and asserted timestamp. It also requires an available
  benchmark and binds the selected contribution/aggregate plus aggregate,
  persisted report-content, PDF, and CSV digests, preventing a delivered
  privacy-suppression notice from passing as quarterly benchmark delivery.
  The benchmark scheduler now selects the prior-quarter-start monthly snapshot
  used by the existing aggregate refresh, labels and bounds the artifact to
  that exact calendar month instead of claiming an unsupported full-quarter
  window; hosted acceptance requires it to be the declared quarter's canonical
  first month rather than accepting a May/June substitute, and the scheduler persists
  the exact contribution and least-coarsened aggregate IDs with the queued
  report. Gate 12 retrieves a service-role-only database attestation that joins
  those IDs to the delivered email and immutable provider event, then computes
  UTC-normalized source dates for the database comparison. Hosted timestamp
  validation accepts PostgreSQL microsecond precision while retaining strict
  timezone and calendar-component checks. The attestation computes
  the aggregate, stored report-body, PDF, and CSV hashes from database state;
  caller-supplied relationship payloads can no longer serve as delivery proof.
  Forward migration `202608060036_benchmark_delivery_attestation.sql` adds the
  persistence and attestation RPCs with browser-role execution revoked.
  Controller QA validates both the disabled baseline and reviewed enabled
  policy states so activation is not blocked by its own preflight suite.
### Changed
- Add a default-disabled, protected Phase 4 hosted acceptance controller with
  independent one-shot switches for Gates 10, 11, and 12. It binds sanitized
  evidence to the exact staging revision, organization, brand, Worker,
