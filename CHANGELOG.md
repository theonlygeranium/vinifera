...
### Fixed
- Make promotion-smoke fixture repositories initialize on an explicit neutral branch so the full test suite is independent of each operator's configured Git default branch. **Deployment impact:** test determinism only; no runtime or hosted state changes.
