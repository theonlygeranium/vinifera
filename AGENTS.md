...
├── supabase/
│   ├── migrations/         # 32 PostgreSQL migrations (do not modify applied migrations)
...
Verification: <commands run and results, e.g. "npm run check; 591/591 Vitest; 155 passed Playwright/axe">
```
...
| `shipcompliant-staging-acceptance.yml` | Manual, protected | Default-disabled exact-revision Gate 13 sandbox acceptance and sanitized evidence; does not provision or deploy the provider |
...
| `vinifera-staging` | `staging` | `vinifera-staging.edstratumlabs.ai` | Isolated self-hosted staging Supabase on Schubert | Protected hosted validation gate |
...
- **Worker API:** `wrangler.jsonc` packages the Express BFF; staging is active and requires `SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY`, `RATE_LIMIT_PEPPER`, `MEMBER_BRAND_CONTEXT_SECRET`, and other server-only bindings. Production remains unactivated.
...
### Current verified test counts (2026-08-06 Gate 13 preparation baseline)
...
| Phase 4 DB (intelligence) | 160 assertions | `npm run qa:db:phase4` |
