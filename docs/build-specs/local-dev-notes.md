...
- The frontend honors `VITE_API_BASE_URL` for credential-free HTTPS or
  ...
  and Vite process are both pinned to browser mode and the loopback Worker, so
  ambient shell or contributor-file values cannot switch to Capacitor routing
  or redirect local traffic.
- Server-generated Auth callbacks and billing returns use canonical
  ...
  environment modes also fail closed. HTTP is accepted only for a loopback
  application origin in those local modes, whether configured or inferred;
  hosted and non-loopback origins require HTTPS.
