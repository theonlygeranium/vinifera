...
**Baseline:** v0.5.0 — BS-01 through BS-06 merged, 568 Vitest + 155 passed Playwright/axe with 3 hosted-only skips
...
- `npm run check` must pass before every push (568 Vitest + TypeScript clean + zero axe violations).
...
npm run check           # TypeScript + 568 Vitest must pass
...
#
#    Verification: npm run check; 568/568 Vitest; 155 passed Playwright/axe
...
| Criterion | Target |
|-----------|--------|
| Vitest suite | 568/568 passing (must not decrease) |
...
|-------|---------------|---------|
| Vitest unit/integration | 568 | `npm run check` |
