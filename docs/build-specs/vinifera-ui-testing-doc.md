...
**Baseline:** v0.5.0 — BS-01 through BS-06 merged, 560 Vitest + 155 passed Playwright/axe with 3 hosted-only skips
...
- `npm run check` must pass before every push (560 Vitest + TypeScript clean + zero axe violations).
...
npm run check           # TypeScript + 560 Vitest must pass
...
#
#    Verification: npm run check; 560/560 Vitest; 155 passed Playwright/axe
...
| Criterion | Target |
|-----------|--------|
| Vitest suite | 560/560 passing (must not decrease) |
...
|-------|---------------|---------|
| Vitest unit/integration | 560 | `npm run check` |
