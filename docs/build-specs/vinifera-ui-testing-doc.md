...
**Baseline:** v0.5.0 — BS-01 through BS-06 merged, 583 Vitest + 155 passed Playwright/axe with 3 hosted-only skips
...
- `npm run check` must pass before every push (583 Vitest + TypeScript clean + zero axe violations).
...
npm run check           # TypeScript + 583 Vitest must pass
...
#
#    Verification: npm run check; 583/583 Vitest; 155 passed Playwright/axe
...
| Criterion | Target |
|-----------|--------|
| Vitest suite | 583/583 passing (must not decrease) |
...
|-------|---------------|---------|
| Vitest unit/integration | 583 | `npm run check` |
