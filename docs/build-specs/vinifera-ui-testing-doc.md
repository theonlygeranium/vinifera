...
**Baseline:** v0.5.0 — BS-01 through BS-06 merged, 584 Vitest + 155 passed Playwright/axe with 3 hosted-only skips
...
- `npm run check` must pass before every push (584 Vitest + TypeScript clean + zero axe violations).
...
npm run check           # TypeScript + 584 Vitest must pass
...
#
#    Verification: npm run check; 584/584 Vitest; 155 passed Playwright/axe
...
| Criterion | Target |
|-----------|--------|
| Vitest suite | 584/584 passing (must not decrease) |
...
|-------|---------------|---------|
| Vitest unit/integration | 584 | `npm run check` |
