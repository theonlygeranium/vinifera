...
**Baseline:** v0.5.0 — BS-01 through BS-06 merged, 566 Vitest + 155 passed Playwright/axe with 3 hosted-only skips
...
- `npm run check` must pass before every push (566 Vitest + TypeScript clean + zero axe violations).
...
npm run check           # TypeScript + 566 Vitest must pass
...
#
#    Verification: npm run check; 566/566 Vitest; 155 passed Playwright/axe
...
| Criterion | Target |
|-----------|--------|
| Vitest suite | 566/566 passing (must not decrease) |
...
|-------|---------------|---------|
| Vitest unit/integration | 566 | `npm run check` |
