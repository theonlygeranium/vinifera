...
**Baseline:** v0.5.0 — BS-01 through BS-06 merged, 556 Vitest + 155 passed Playwright/axe with 3 hosted-only skips
...
- `npm run check` must pass before every push (556 Vitest + TypeScript clean + zero axe violations).
...
npm run check           # TypeScript + 556 Vitest must pass
...
#
#    Verification: npm run check; 556/556 Vitest; 155 passed Playwright/axe
...
| Criterion | Target |
|-----------|--------|
| Vitest suite | 556/556 passing (must not decrease) |
...
|-------|---------------|---------|
| Vitest unit/integration | 556 | `npm run check` |
