...
**Baseline:** v0.5.0 — BS-01 through BS-06 merged, 601 Vitest + 155 passed Playwright/axe with 3 hosted-only skips
...
- `npm run check` must pass before every push (601 Vitest + TypeScript clean + zero axe violations).
...
npm run check           # TypeScript + 601 Vitest must pass
...
#
#    Verification: npm run check; 601/601 Vitest; 155 passed Playwright/axe
...
| Criterion | Target |
|-----------|--------|
| Vitest suite | 601/601 passing (must not decrease) |
...
|-------|---------------|---------|
| Vitest unit/integration | 601 | `npm run check` |
