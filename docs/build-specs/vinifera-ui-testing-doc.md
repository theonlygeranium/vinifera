...
**Baseline:** v0.5.0 — BS-01 through BS-06 merged, 602 Vitest + 155 passed Playwright/axe with 3 hosted-only skips
...
- `npm run check` must pass before every push (602 Vitest + TypeScript clean + zero axe violations).
...
npm run check           # TypeScript + 602 Vitest must pass
...
#
#    Verification: npm run check; 602/602 Vitest; 155 passed Playwright/axe
...
| Criterion | Target |
|-----------|--------|
| Vitest suite | 602/602 passing (must not decrease) |
...
|-------|---------------|---------|
| Vitest unit/integration | 602 | `npm run check` |
