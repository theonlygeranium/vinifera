...
**Baseline:** v0.5.0 — BS-01 through BS-06 merged, 588 Vitest + 155 passed Playwright/axe with 3 hosted-only skips
...
- `npm run check` must pass before every push (588 Vitest + TypeScript clean + zero axe violations).
...
npm run check           # TypeScript + 588 Vitest must pass
...
#
#    Verification: npm run check; 588/588 Vitest; 155 passed Playwright/axe
...
| Criterion | Target |
|-----------|--------|
| Vitest suite | 588/588 passing (must not decrease) |
...
|-------|---------------|---------|
| Vitest unit/integration | 588 | `npm run check` |
