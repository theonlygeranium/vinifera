...
**Baseline:** v0.5.0 — BS-01 through BS-06 merged, 559 Vitest + 155 passed Playwright/axe with 3 hosted-only skips
...
- `npm run check` must pass before every push (559 Vitest + TypeScript clean + zero axe violations).
...
npm run check           # TypeScript + 559 Vitest must pass
...
#
#    Verification: npm run check; 559/559 Vitest; 155 passed Playwright/axe
...
| Criterion | Target |
|-----------|--------|
| Vitest suite | 559/559 passing (must not decrease) |
...
|-------|---------------|---------|
| Vitest unit/integration | 559 | `npm run check` |
