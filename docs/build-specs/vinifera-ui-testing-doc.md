...
**Baseline:** v0.5.0 — BS-01 through BS-06 merged, 547 Vitest + 155 passed Playwright/axe with 3 hosted-only skips
...
- `npm run check` must pass before every push (547 Vitest + TypeScript clean + zero axe violations).
...
npm run check           # TypeScript + 547 Vitest must pass
...
#
#    Verification: npm run check; 547/547 Vitest; 155 passed Playwright/axe
...
| Criterion | Target |
|-----------|--------|
| Vitest suite | 547/547 passing (must not decrease) |
...
| Phase 5 DB | 514 assertions | `npm run qa:db:phase5` |
