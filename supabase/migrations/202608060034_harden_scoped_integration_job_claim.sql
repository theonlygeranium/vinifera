-- Replace the eligible global-queue claim surface with an exact-ID Gate 15
-- quarantine claim. Synthetic jobs remain future-dated and therefore
-- ineligible to normal workers until this function atomically leases them.
-- This is forward-only because migration 202608060033 may already be applied.

drop function if exists public.claim_integration_sync_jobs_for_scope(
...
begin
  ...
  with candidates as (
    ...
    where job.organization_id = p_organization_id
      ...
      and job.payload ->> 'hosted_acceptance' = 'gate15'
...
revoke all on function public.claim_gate15_integration_sync_jobs_for_scope(
  uuid, uuid[], uuid[], text, integer, integer, timestamptz
) from public, anon, authenticated;

grant execute on function public.claim_gate15_integration_sync_jobs_for_scope(
  uuid, uuid[], uuid[], text, integer, integer, timestamptz
) to service_role;
