...
export async function getRuntimeConfigurationReport(
  ...
  const supabaseOrigin = normalizedOrigin(env.SUPABASE_URL);
  if (supabaseOrigin) {
    report.database.bindingHashes = {
      supabaseUrlSha256: await sha256(supabaseOrigin),
    };
  }
  if (env.COMPLIANCE_PROVIDER !== "shipcompliant") return report;
  const values = {
    accountIdSha256: env.SHIPCOMPLIANT_ACCOUNT_ID,
    checkPathSha256: env.SHIPCOMPLIANT_CHECK_PATH,
    contractVersionSha256: env.SHIPCOMPLIANT_CONTRACT_VERSION,
    licenseIdSha256: env.SHIPCOMPLIANT_LICENSE_ID,
    sandboxOriginSha256: normalizedOrigin(env.SHIPCOMPLIANT_BASE_URL),
    tokenPathSha256: env.SHIPCOMPLIANT_TOKEN_PATH,
  };
  if (Object.values(values).every((value) => typeof value === "string" && value)) {
    report.compliance.bindingHashes = Object.fromEntries(
      await Promise.all(
        Object.entries(values).map(async ([name, value]) => [
          name,
          await sha256(String(value)),
        ]),
      ),
    );
  }
